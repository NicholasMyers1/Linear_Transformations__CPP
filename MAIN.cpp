...
void display(){
	//3d for stuff in window: 
		glEnable(GL_DEPTH_TEST);		glEnable(GL_COLOR_MATERIAL);
		...
		glMatrixMode(GL_MODELVIEW);
			...
			glEnable(GL_BLEND);glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
				if(WINDOW.currentScreen==2)DRAW3D();
	//2d for buttons: 
		glDisable(GL_DEPTH_TEST);		glDisable(GL_LIGHTING);		glDisable(GL_CULL_FACE);	glEnable(GL_BLEND);glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
		glMatrixMode(GL_PROJECTION);	glLoadIdentity();
...
//		glutPositionWindow((glutGet(GLUT_SCREEN_WIDTH)-glutGet(GLUT_SCREEN_WIDTH)/1.25)/2,(glutGet(GLUT_SCREEN_HEIGHT)-glutGet(GLUT_SCREEN_HEIGHT)/1.25)/2);
		//BUTTON[1][WINDOW.buttonCurrentlyClicked].text=OBJECTS.transformationFactor[WINDOW.buttonCurrentlyClicked%2][WINDOW.buttonCurrentlyClicked==3||WINDOW.buttonCurrentlyClicked==4?1:0];
//	}
//	else{
		glutFullScreen();
		//BUTTON[1][WINDOW.buttonCurrentlyClicked].text=OBJECTS.transformationFactor[WINDOW.buttonCurrentlyClicked%2][WINDOW.buttonCurrentlyClicked==3||WINDOW.buttonCurrentlyClicked==4?1:0];
...
void reshape(int w,int h){
	WINDOW.winW=w;		WINDOW.winH=h;
	static double winWI=WINDOW.winW;		WINDOW.aspect=WINDOW.winW/winWI;
	BUTTONS_INIT();//everytime the WINDOW resizes, redraw the buttons correctly;
	glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
			glViewport(0,0,w,h);		gluPerspective(60.f,(GLfloat)w/(GLfloat)h,0.01f,256.0f);	glMatrixMode(GL_MODELVIEW);		glLoadIdentity();
}//end void;
int main(int argc,char **argv){//main and set window properties;
    ...
    glutInit(&argc,argv);
		...
		glutCreateWindow("Linear Transformations");
